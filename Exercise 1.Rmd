...
p1 <- ggplot() + geom_line(aes(x = x, y= y,col = "red"), data = as.data.frame(cbind(x,y)), size = 1)
y <- y <- sapply(x, function(xx) matern.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.matern[2]))
p1 <- p1 + geom_line(aes(x = x, y= y, col = "blue"), data = as.data.frame(cbind(x,y)), size = 1)
...
p <- p + geom_point(aes(x=variogram4$u, y = variogram4$v, colour = "Realization 4"), size = 1) +
  geom_line(aes(x=variogram4$u, y = variogram4$v, colour = "Realization 4"), suze = 1) + 
  theme_classic()
...
summary(likfit.36)
```
We thus can conclude that using ML one will get an estimate $\hat\sigma^2 =$ `r likfit.36$sigmasq`. And estiamte $\hat \xi =$ `r likfit.36$phi` on the realization. Which is not too far off from the original values.
