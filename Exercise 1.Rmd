...
```{r fig1d1, fig.height=4, fig.width=7, fig.cap="\\label{fig:fig1d1} Posterior estimation, matern $\\sigma^2 = 5$, $\\nu = 3$", fig.align="center" }
  ...
  p <- ggarrange(p1,p2, ncol=2, widths = c(1,1), heights = c(1,1), nrow = 1)
  p <- annotate_figure(p, top = text_grob("Matern Posterior Realisation", size = 17))
  p
...
```{r fig3a1, fig.height=10, fig.width=5, fig.cap="\\label{fig:fig3a1} Realizations of field with $\\xi = 3$, $\\sigma^2 = 2$", fig.align="center" }
...
summary(likfit.36)
```
We get an estimate $\hat\sigma^2 =$ `r likfit.36$sigmasq`. And estiamte $\hat \xi =$ `r likfit.36$phi`
...
p2 <- l.fit(9, colour = "Red")$p
p3 <- l.fit(64, colour = "Green")$p
p4 <- l.fit(100, colour = "Purple")$p
