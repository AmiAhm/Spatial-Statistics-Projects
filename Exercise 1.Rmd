...
p1 <- ggplot() + geom_line(aes(x = x, y= y), colour = "red", data = as.data.frame(cbind(x,y)), size = 1)
y <- y <- sapply(x, function(xx) matern.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.matern[2]))
p1 <- p1 + geom_line(aes(x = x, y= y), colour = "blue", data = as.data.frame(cbind(x,y)), size = 1)
p1 <- p1 + ggtitle("a) Matern") + xlab(TeX("$\\tau$")) + ylab(TeX("$\\rho_r(\\tau)$"))
p1 <- p1 +
  scale_color_manual(values = c("red", "blue"),labels = unname(TeX(c(paste("$\\nu =$", vs.matern[1])
                                             ,paste("$\\nu =$", vs.matern[2]))))) +
  theme_classic() +
...
p2 <- ggplot() + geom_line(aes(x = x, y= y,col = "red"), data = as.data.frame(cbind(x,y)), size = 1)
y <- y <- sapply(x, function(xx) powered.exp.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.exponential[2]))
p2 <- p2 + geom_line(aes(x = x, y= y, col = "blue"), data = as.data.frame(cbind(x,y)), size = 1)
...
p4 <- p4 + ggtitle(TeX("b) Powered exponential")) + xlab(TeX("$\\tau$")) + ylab(TeX("$\\gamma_r(\\tau)$")) +
  scale_color_manual(values = c("red", "purple", "green", "blue"),labels = unname(TeX(c(paste("$\\nu =$", vs.exponential[1], ", $\\sigma^2 =$",sigma2s[2])
...
# Problem 3f
Using maximum likelihood to estiamte parameters seems to be relatively fast using the number of parameters we have here, and with enough data points it seems to be quite good at finding the correct parameters.
