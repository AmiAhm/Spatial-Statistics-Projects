...
$$Cov(\vect d(\vect x), \vect l(\vect x')) = ...
...
The integral 
$$\int_{0}^{\infty}p(\vect d(\vect x)|\sigma_r^2, \sigma_e^2) d\sigma^2$$ goes to infinity, thus not a pdf. If we try to draw sigmas beliving L is a pdf, we would expect values to goes to infinity, which would give us more variance than we have. 

# Problem 1d) 
$\vect d(\vect x)$ and $\vect l(\vect x)$ are both multivariate gaussian so $\vect l | \vect d$ would also be. 
Have: 
