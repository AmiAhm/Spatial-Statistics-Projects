...
author: "Amir Ahmed - Candidate Number: 10003"
#date: "1/23/2020"
output:
  pdf_document:
    fig_caption: yes
...
y1 <- sapply(x, function(xx) matern.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.matern[1]))
df1 <- cbind(x, y1)
y2<- sapply(x, function(xx) matern.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.matern[2]))
df2 <- cbind(x, y2)
...
df1$v = "v = 1"
df2$v = "v = 3"
colnames(df1) <- c("x", "y", "v")
colnames(df2) <- c("x", "y", "v")
df <- rbind(df1, df2)
df$v <- as.factor(df$v)

p1 <- ggplot() + geom_line(aes(x = x, y= y, colour = v), data = df, size = 1)
p1 <- p1 + ggtitle("a) Matern") + xlab(TeX("$\\tau$")) + ylab(TeX("$\\rho_r(\\tau)$"))
p1 <- p1 +
  theme_classic() +
...
y1 <- sapply(x, function(xx) powered.exp.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.exponential[1]))
df1 <- cbind(x, y1)
y2<- sapply(x, function(xx) powered.exp.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.exponential[2]))
df2 <- cbind(x, y2)
...
p2 <- ggplot() + geom_line(aes(x = x, y= y, colour = v), data = df, size = 1)
p2 <- p2 + ggtitle(TeX("b) Powered exponential")) + xlab(TeX("$\\tau$")) + ylab(TeX("$\\rho_r(\\tau)$"))
p2 <- p2 +
  theme_classic() +
  theme(legend.key.size = unit(1.5, 'lines'),
        legend.position = 'top',
...
p3 <- ggplot() + geom_line(aes(x = x, y= y,col = "red"), data = as.data.frame(cbind(x,y)), size = 1)
y <- sigma2s[1]*(1- sapply(x, function(xx) matern.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.matern[2])))
p3 <- p3 + geom_line(aes(x = x, y= y,col = "purple"), data = as.data.frame(cbind(x,y)), size = 1 , linetype = "dashed")
y <- sigma2s[2]*(1-  sapply(x, function(xx) matern.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.matern[1])))
p3 <- p3 + geom_line(aes(x = x, y= y,col = "green"), data = as.data.frame(cbind(x,y)), size = 1)
y <- sigma2s[2]*(1-  sapply(x, function(xx) matern.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.matern[2])))
p3 <- p3 + geom_line(aes(x = x, y= y,col = "blue"), data = as.data.frame(cbind(x,y)), size = 1 , linetype = "dashed")
p3 <- p3 + ggtitle(TeX("a) Matern")) + xlab(TeX("$\\tau$")) + ylab(TeX("$\\gamma_r(\\tau)$")) +
  scale_color_manual(values = c("red", "purple", "green", "blue"),labels = unname(TeX(c(
...
p4 <- ggplot() + geom_line(aes(x = x, y= y,col = "red"), data = as.data.frame(cbind(x,y)), size = 1)
y <- sigma2s[1]*(1- sapply(x, function(xx) powered.exp.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.exponential[2])))
p4 <- p4 + geom_line(aes(x = x, y= y,col = "purple"), data = as.data.frame(cbind(x,y)), size = 1 , linetype = "dashed")
y <- sigma2s[2]*(1-  sapply(x, function(xx) powered.exp.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.exponential[1])))
p4 <- p4 + geom_line(aes(x = x, y= y,col = "green"), data = as.data.frame(cbind(x,y)), size = 1)
y <- sigma2s[2]*(1-  sapply(x, function(xx) powered.exp.cov.fun(tau = xx, sigma2 = sigma2s[1], nu = vs.exponential[2])))
p4 <- p4 + geom_line(aes(x = x, y= y,col = "blue"), data = as.data.frame(cbind(x,y)), size = 1, linetype = "dashed")
...
n.realizations <- function(n, xx, sigma2, nu, cov.fun, tau.fun, title = "title"){
  ...
  pl <- ggplot() + theme_classic() +
    ...
    ylim(c(-7, 7)) + 
    xlim(c(0, 50))
  pl <- pl + geom_line(data = observations, aes(x = x, y = observed_value, color = trial), size = 1) + ggtitle(TeX(paste(title, "$\\nu =$", nu, ", $\\sigma^2 =$",sigma2))) +
     scale_color_discrete(labels = c("1", "2", "3", "4")
                          ,guide = guide_legend(label.hjust = 0.1, title="Trial number: ")
     )
...
p4 <- res$p
matern_dat <- res$dat
plot1 <- ggarrange(p1,p2, p3, p4, ncol = 2, nrow = 2, common.legend=T, legend ="bottom")  + theme(legend.title = element_blank())
plot1 <- annotate_figure(plot1, top = text_grob("Matern prior realization", size = 17))
plot1
...
plot2 <- ggarrange(p5, p6, p7, p8, ncol = 2, nrow = 2,  common.legend=T, legend ="bottom")  + theme(legend.title = element_blank())
plot2 <- annotate_figure(plot2, top = text_grob("Powered exponential prior realization", size = 17))
plot2
...
As $\epsilon(x)$ and $r(x)$ are Gaussian, a linear product of the two would also be. 
...
```{r fig1d1, fig.height=4, fig.width=7, fig.cap="\\label{fig:fig1d1} Posterior estimation, matern $\\sigma^2 = 5$, $\\nu = 3$", fig.align="center" }
  ...
  generate_plot <- function(title, sigma_e_2){
    ...
    colors <- c("90% theoretical confidence" = "skyblue", "Theoretical mean" = "black", "Actual path" = "red", "Prior observation" = "black")
    observed <- c("Prior observation" = "dotted")


    p <- ggplot(data = as.data.frame(cbind(xx, mu.r.d., min.90, max.90, observations))) +
      geom_vline(aes(xintercept = dd[1], linetype = "Prior observation"), size = 1) + 
      geom_vline(aes(xintercept = dd[2], linetype = "Prior observation"), size = 1) + 
      geom_vline(aes(xintercept = dd[3], linetype = "Prior observation"), size = 1) + 
      geom_ribbon(aes(x = xx, ymin = min.90, ymax = max.90, fill = "90% theoretical confidence"), alpha = 0.5) + 
      geom_line(aes(x = xx, y = mu.r.d., color = "Theoretical mean"), size = 1) +
      ...
      geom_line(aes(x = xx, y = matern_dat[,2], color = "Actual path"), size = 1) +# The actual realisation
      ggtitle(title) +
      scale_fill_manual("", values = colors) +
      scale_color_manual("", values = colors) +
      scale_linetype_manual("", values = observed) +
      xlab("x") +
      ylab("r|d") + 
      xlim(c(0,50)) + 
      ylim(c(-6, 6)) + 
  ...
  p <- ggarrange(p1,p2, ncol=2, widths = c(1,1), heights = c(1,1), nrow = 1, common.legend=T, legend ="bottom")  + theme(legend.title = element_blank())
...
```{r fig1e1, fig.height=4, fig.width=7, fig.cap="\\label{fig:fig1e2} Posterior estimation 100 realisation, matern $\\sigma^2 = 5$, $\\nu = 3$", fig.align="center" }
generate_plot2 <- function(title, sigma_e_2){
  ...
  colors <- c("90% theoretical confidence" = "skyblue", "Theoretical mean" = "black", "Actual path" = "red", "Prior observation" = "black", "90% empirical confidence" = "green", "Empirical mean" = "blue")
  observed <- c("Prior observation" = "dotted")
  ...
  p <- ggplot(data = observations) +
    ...
    geom_vline(aes(xintercept = dd[2], linetype = "Prior observation"), size = 1) + 
    geom_vline(aes(xintercept = dd[3], linetype = "Prior observation"), size = 1) + 
    geom_ribbon(aes(x = x, ymin = emp.min_90, ymax = emp.max_90, fill = "90% empirical confidence"), alpha = 0.5) + 
    geom_ribbon(aes(x = xx, ymin = min.90, ymax = max.90, fill = "90% theoretical confidence"), alpha = 0.5) + 
    # Theoretical values
    geom_line(aes(x = xx, y = mu.r.d., color = "Theoretical mean"), size = 1) +
    geom_line(aes(x = xx, y = matern_dat[,2], color = "Actual path"), size = 1) +# The actual realisation +
    geom_line(aes(x = x, y = emp_mu, color = "Empirical mean"), size = 1) +
    ...
    theme(legend.title = element_blank()) + 
    theme_classic()  + 
    guides(fill=guide_legend(nrow=2,byrow=TRUE), line=guide_legend(nrow=2,byrow=TRUE)) + 
    ylim(-6, 6) + 
    xlim(0, 50)
  #p
  list(p=p, draw = draw, mu.r.d.= mu.r.d.)
}
p1 <- generate_plot2("a) No observation error", 0)$p
p2 <- generate_plot2("b) Observation error", 0.25)$p
p <- ggarrange(p1,p2, ncol=2, widths = c(1,1), heights = c(1,1), nrow = 1, common.legend=T, legend ="bottom")  + theme(legend.title = element_blank())
p <- annotate_figure(p, top = text_grob("Empirical estimation w/ 100 realizations", size = 17))
p
...
est_int <- apply(draw, 2, function(x) sum((x>2)*(x-2)))
mean_length <- mean(est_int)
var_length <- var(est_int)
...
probs <- pnorm(2, mean = mu, sd = deviatons, lower.tail = F) 
...
colors <- c("90% theoretical confidence > 2" = "green", "90% theoretical confidence < 2" = "skyblue", "Actual path" = "red", "Prior observation" = "black", "Theoretical P(I(r(x)>2)" = "red", "Empirical P(I(r(x)>2)" = "blue", "Posterior, theoretical mean" = "black")
observed <- c("Prior observation" = "dotted", "y=2" = "solid")
#### Plotting:
p <- ggplot() + 
  ylim(c(-5,5)) +
  geom_line(aes(x = xx, y = mu.r.d., colour = "Posterior, theoretical mean"), size = 1) +
  geom_ribbon(aes(x = xx, ymin = min.90.2, ymax = max.90.2, fill = "90% theoretical confidence > 2"), alpha = 0.5) +
  geom_ribbon(aes(x = xx, ymin = min.90.3, ymax = max.90.3, fill = "90% theoretical confidence < 2"), alpha = 0.5) +
  geom_vline(aes(xintercept = dd[1], linetype = "Prior observation"), size = 1) + 
  geom_vline(aes(xintercept = dd[2], linetype = "Prior observation"), size = 1) + 
  geom_vline(aes(xintercept = dd[3], linetype = "Prior observation"), size = 1) + 
  geom_hline(aes(yintercept = 2, linetype = "y=2")) +
  geom_line(aes(x = grouped_over_2$V1, y = grouped_over_2$perc_over_2*plot.height + plot.min, colour = "Empirical P(I(r(x)>2)"), size = 1)+
  geom_line(aes(x = xx, y =probs, colour = "Theoretical P(I(r(x)>2)"), size = 1)+
  xlim(0, 50) +
  theme_classic() + 
  xlab("x") + 
  ylab("r|d") + 
  ...
  scale_color_manual("", values = colors) +
  scale_linetype_manual("", values = observed) +
  ggtitle(TeX("Posterior with $I(r(x)>2)$")) + 
  theme(legend.title = element_blank()) 
...
The situation with $I(r(x)>2)$ is displayed in Figure \ref{fig:fig1f1}.


A prediction of the integral using our 100 realizations from the posterior model with $\sigma_\epsilon^2$ is:
...
Applying this to our realisation we get: $\hat A_r =$ `r mean_length` and $Var(\hat A) =$ `r var_length`

Another estimator is: 
\begin{equation}
    \tilde A_r = \sum_{x \in L} I(\hat r(x) > 2)(\hat r(x) - 2)
\end{equation}
With our data we get:  $\tilde A_r =$  `r tilde.A`
...
g3 <- ggplot(data=topo) + geom_histogram(mapping=aes(x=topo$z, y = ..density..)) + theme_classic()+xlab("z")#+ geom_frequency(mapping = aes(x=topo$z, y= ..density..))
...
```{r  fig4, echo=FALSE, fig.asp=1, fig.cap="\\label{fig:fig4} Predictions and associated variance, fig.height=0.5"}
...
```{r fig3a1, fig.height=12, fig.width=12, fig.cap="\\label{fig:fig3a1} Realizations of field with $\\xi = 3$, $\\sigma^2 = 2$", fig.align="center" }
...
colors <- c("Realization 1" = "Blue", "Realization 2" = "Red", "Realization 3" = "Green", "Realization 4" = "Purple", "Actual variogram" = "black")
M1 <- as.matrix(cbind(obs.points[,1], obs.points[,2], draw1))
geo1 <- as.geodata(M1)
variogram1 <- variog(geo1)
p <- ggplot() + geom_point(aes(x=variogram1$u, y = variogram1$v, colour = "Realization 1"), size = 1) +
  geom_line(aes(x=variogram1$u, y = variogram1$v, colour = "Realization 1"), size = 1)

M2 <- as.matrix(cbind(obs.points[,1], obs.points[,2], draw2))
geo2 <- as.geodata(M2)
variogram2 <- variog(geo2)
p <- p + geom_point(aes(x=variogram2$u, y = variogram2$v, colour = "Realization 2"), size = 1)  +
  geom_line(aes(x=variogram2$u, y = variogram2$v, colour = "Realization 2"), size = 1)


M3 <- as.matrix(cbind(obs.points[,1], obs.points[,2], draw3))
geo3 <- as.geodata(M3)
variogram3 <- variog(geo3)
p <- p + geom_point(aes(x=variogram3$u, y = variogram3$v, colour = "Realization 3"), size = 1) +
  geom_line(aes(x=variogram3$u, y = variogram3$v, colour = "Realization 3"), size = 1)

M4 <- as.matrix(cbind(obs.points[,1], obs.points[,2], draw4))
geo4 <- as.geodata(M4)
variogram4 <- variog(geo4)
p <- p + geom_point(aes(x=variogram4$u, y = variogram4$v, colour = "Realization 4"), size = 1) +
  geom_line(aes(x=variogram4$u, y = variogram4$v, colour = "Realization 4"), size = 1) + 
  theme_classic()
...
p <- p + geom_line(aes(x = xx, y = yy, colour = "Actual variogram"), size = 1)
...
```{r include=FALSE}
# Problem 3d)
l.fit <- function(n, colour = "Black", title = ""){
colors <- c("Empirical variogram" = colour, "Theoretical variogram w/ estimated parameters" = colour, "Actual theoretical variogram" = "black")
observed <- c("Theoretical variogram w/ estimated parameters" = "dashed")
  ...
  p <- ggplot() + 
    geom_line(aes(x = xx, y = yy.theoretical, colour = "Actual theoretical variogram")) + 
    geom_point(aes(x=variogram1$u, y = variogram1$v, colour = "Empirical variogram")) +
    geom_line(aes(x=variogram1$u, y = variogram1$v, colour = "Empirical variogram")) +
    geom_line(aes(x = xx, y = yy, linetype = "Theoretical variogram w/ estimated parameters")) +
    ggtitle(paste(title, " ", n, "observations")) +
    theme_classic() + 
    xlab(TeX("$\\tau$")) + 
    ylab(TeX("$\\gamma_r(\\tau)$")) + 
    scale_fill_manual("", values = colors) +
...
res <- l.fit(36,  colour = "Red", title = "b)")
likfit.36 <- res$likfit
```
...
Using the 36 point we estimate $\hat\sigma^2 =$ `r likfit.36$sigmasq` and  $\hat \xi =$ `r likfit.36$phi`.
We repeat the process in 3d for $n=6$, $n=64$, $n=100$. 

```{r include=FALSE}
# Problem 3e)
res2 <- l.fit(9, colour = "Red", title = "a)")
res3 <- l.fit(64, colour = "Red", title = "c)")
res4 <- l.fit(100, colour = "Red", title = "d)")
...
```{r fig.cap="\\label{fig:fig3d2} Variograms from uniform drawn points $\\xi = 3$, $\\sigma^2 = 2$, maximum likelihood fit"}
p1 <- res2$p 
p2 <- res$p
p3 <- res3$p
p4 <- res4$p
p <- ggarrange(p1, p2,p3,p4, nrow = 2, ncol = 2, common.legend = T, legend = "bottom")
p
```

These variograms are displayed in Figure \ref{fig:fig3d2}. They seem to vary quite much, but that can probably be blamed on a low amount of observation points. As we draw a very limited amount of points, some distances will be less represented than others, which will cause large variations in estimation. This can especially be seen for $n=9$.
